...
\textbf{Response:} The figures in question are pipeline and logic diagrams stored as PDF vector files (Figures 5.1, 5.3, 5.4, 5.5, 5.6) and a hazard ratio forest plot stored as a PNG (Figure 6.3). The small font appearance arose because the source diagrams were generated at a resolution where the text labels were small relative to the figure canvas. Simply scaling the figures in LaTeX enlarges the canvas but does not increase the font size relative to the printed page. To address this, each affected figure was re-rendered at 400 DPI (four times the standard screen resolution), and the resulting high-resolution raster was converted back to PDF at 300 DPI with the page dimensions set to match the thesis text width. This ensures that when LaTeX includes the figure at \texttt{\textbackslash linewidth}, the text labels are physically larger on the printed page. The PNG figures (Figure 6.3 and the ML result figures) were upscaled to 1875 pixels wide using Lanczos resampling at 300 DPI. The figures now render with legible labels at the printed page size.
...
\begin{enumerate}
    \item
    \begin{enumerate}
        \item Figure 5.1 (MIMIC-IV data universe): Page 33. Re-rendered at 400 DPI and converted to PDF at 300 DPI. Figure now included at \texttt{width=1.1\textbackslash linewidth} with \texttt{\textbackslash makebox} centering.
    \end{enumerate}
    \item
    \begin{enumerate}
        \item Figure 5.3 (CONSORT flow diagram): Page 35. Same re-rendering treatment applied.
    \end{enumerate}
    \item
    \begin{enumerate}
        \item Figure 5.4 (Sepsis-3 logic and urine output sessionization, two-panel): Page 36. Both subfigures re-rendered at 400 DPI. Subfigure widths adjusted from \texttt{0.49\textbackslash linewidth} to \texttt{0.48\textbackslash linewidth} to prevent overflow.
    \end{enumerate}
    \item
    \begin{enumerate}
        \item Figure 5.5 (patient timeline): Page 37. Re-rendered at 400 DPI and converted to PDF at 300 DPI.
    \end{enumerate}
    \item
    \begin{enumerate}
        \item Figure 5.6 (statistical testing results): Page 39. Re-rendered at 400 DPI.
    \end{enumerate}
    \item
    \begin{enumerate}
        \item Figure 6.3 (Cox PH forest plot): Page 51. Upscaled from 1189 to 1875 pixels wide using Lanczos resampling at 300 DPI. LaTeX inclusion width increased from \texttt{0.7\textbackslash textwidth} to \texttt{0.85\textbackslash textwidth}.
    \end{enumerate}
\end{enumerate}
